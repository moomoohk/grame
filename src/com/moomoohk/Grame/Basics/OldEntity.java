 ...
 * @since 2013-04-05
 */
public class OldEntity extends GrameObject
{
	 ...
	 */
	public OldEntity(EntityGenerator entGen)
	{
		this(entGen.nameGen(), entGen.typeGen(), 1, GrameUtils.randomColor());
	}

	public OldEntity(String name)
	{
		this(name, "", 0, GrameUtils.randomColor());
	}

	public OldEntity(String name, Color color)
	{
		this(name, "", 0, color);
	}
	 ...
	 *            Entity color.
	 */
	public OldEntity(String name, String type, int level, Color color)
	{
	...
	public void tick(int bID)
	{
		...
		if (this.activeAI.size() != 0 && this.activeAI.get(bID) != null)
			c = this.activeAI.get(bID).getNext(getPos(bID), target, GrameManager.findBase(bID), this, (OldEntity) GrameManager.findGrameObject(targetID));
		setPos(bID, c);
	}
	...
	private void determineAI(int bID)
	{
		...
		if (this.overrideAI.size() == 0 || this.overrideAI.get(bID) == null)
		{
			...
			for (int i = 0; i < this.AI.get(bID).size(); i++)
			{
				...
				if (!this.AI.get(bID).get(i).isValid(getPos(bID), target, GrameManager.findBase(bID), this, (OldEntity) GrameManager.findGrameObject(targetID)))
					continue;
	 ...
	 * @see OldEntity#makePlayer(int, boolean, int)
	 */
